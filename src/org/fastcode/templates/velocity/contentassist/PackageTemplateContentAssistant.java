...
public class PackageTemplateContentAssistant extends AbstractTemplateContentAssistant {
	...
	public PackageTemplateContentAssistant(final Map<FIRST_TEMPLATE, SECOND_TEMPLATE> properties, final boolean propertiesOnly) {
	...
	public List<ICompletionProposal> getCompletionProposals(final IDocument document, final int offset, final int length, final String spaceToPad) {
		try {
			...
			if (element != null) {
				return TemplateManager.getCompletionProposals(element, offset, length, this.properties, this.propertiesOnly, FIRST_TEMPLATE.Package);
			}
